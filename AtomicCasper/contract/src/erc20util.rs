...
use crate::{utils::{get_key_val, ToKey, get_contract_package_hash}, constants, transfers::U512ToU256, Error};
pub fn get_own_contract_balance() -> u64{
    ...
    runtimeargs.insert("address", own_contract_hash_key).unwrap_or_revert_with(Error::RuntimeArgFailed);
...
pub fn transfer_erc20_tokens_to(reciver : Key){
    ...
    let erc20_contract_hash = get_key_val::<ContractHash>(constants::NAMED_KEY_CONTRACT_HASH);
    let mut runtimeargs = RuntimeArgs::new();
    runtimeargs.insert("amount", amount).unwrap_or_revert_with(Error::RuntimeArgFailed);
